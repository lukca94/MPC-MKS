...
{
	__disable_irq();
	uint32_t precision_tick = __HAL_TIM_GET_COUNTER(&htim2);
	__enable_irq();
...
{
	while (get_precision_tick() < target_ticks) { /* do nothing */ }
}
...
void send(uint8_t *buffer)
{
	...
	rfm95_send_receive_cycle(&rfm95_handle, buffer, strlen(buffer));
...
int main(void)
{
	...
	while (1)
	{
		...
		{
			memcpy(buffer, "246892;press", sizeof("246892;press"));
			send(&buffer);
		}

		if ((HAL_GetTick() - lastTimer) > 30000)
		{
			counter++;

			sprintf(msg,"246892;%i",counter);
