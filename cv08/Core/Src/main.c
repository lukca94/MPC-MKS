...
CheckStatus_t CodeCheck(char key)
{
	static int position = 0;
	static uint32_t timePressed;

	if(code[position] == key)
	{
		if ((position > 0) && ((HAL_GetTick()-timePressed) > 3000))
		{
			position = 0;
			return TIMEOUT;
		}
		else if(position == 4) //CODE OK
		{
			position = 0;
			return CORRECT_CODE;
		}
